...
for g = 1:num_groups
    ...
    % Padding: unfilled source rows stay zero
...
% Encode one column (K elements) to R parity elements over GF(256)
% Vandermonde matrix: V(i,j) = j^(i-1) for i=1..R, j=1..K
parity = zeros(R, 1, 'uint8');
for i = 1:R
    sum = 0;
    for j = 1:K
        if src_col(j) ~= 0
            % Multiply src_col(j) * j^(i-1) in GF(256)
...
function inv = gf256_inv(a, exp_table, log_table)
if a == 0
    inv = 0;
else
...
% Generate GF(256) exp/log tables with primitive polynomial x^8+x^4+x^3+x^2+1
...
for i = 0:254
    ...
    x = bitshift(x, 1);
    if bitand(x, 256)
        x = bitxor(x, 285);  % 0x11D = x^8 + x^4 + x^3 + x^2 + 1
    end
    x = bitand(x, 255);
end
exp_table(256) = exp_table(1);  % wrap
end
